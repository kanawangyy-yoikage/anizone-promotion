...
(function () {
  ...
  class Particle {
    ...
    update() {
      ...
      if (this.life > this.maxLife || this.x < -5 || this.x > W+5 || this.y < -5 || this.y > H+5) this.reset();
    }
    draw() {
  ...
  class ShootingStar {
    constructor() {
      ...
      this.len=80+Math.random()*120; this.speed=5+Math.random()*8;
      this.alpha=0; this.alive=true; this.life=0; this.maxLife=50+Math.random()*30;
    }
    update() {
      this.x+=this.speed*1.2; this.y+=this.speed*0.5; this.life++;
  ...
  let stars=[]; let starTimer=0;
  function drawFrame() {
    ctx.clearRect(0,0,W,H);
    particles.forEach(p=>{p.update();p.draw();});
    starTimer++;
    if(starTimer>220+Math.random()*200){stars.push(new ShootingStar());starTimer=0;}
  ...
  if (navbar) {
    navbar.style.transition = 'transform 0.4s cubic-bezier(0.4,0,0.2,1), background 0.3s, box-shadow 0.3s';
  }
  ...
  window.addEventListener('scroll', () => {
    ...
    if (navbar) {
      ...
      if (y > lastScrollY + 4 && y > 120) navbar.style.transform = 'translateY(-100%)';
      else if (y < lastScrollY - 4)        navbar.style.transform = 'translateY(0)';
      lastScrollY = y;
    }
  }, { passive: true });
  ...
  function addTilt(cards) {
    cards.forEach(card => {
      card.addEventListener('mousemove', (e) => {
        ...
        card.style.transform=`translateY(-6px) scale(1.01) perspective(600px) rotateX(${-dy*5}deg) rotateY(${dx*5}deg)`;
      });
      card.addEventListener('mouseleave', () => {
        card.style.transform='';
        card.style.transition='all 0.5s cubic-bezier(0.34,1.56,0.64,1)';
      });
      card.addEventListener('mouseenter', () => { card.style.transition='all 0.15s ease'; });
    });
  }
  ...
  document.querySelectorAll('.feature-card, .flow-box, .dev-card').forEach(card => {
    card.addEventListener('mousemove', (e) => {
      ...
      card.style.background=`radial-gradient(circle at ${x}% ${y}%, rgba(255,255,255,0.09) 0%, rgba(255,255,255,0.03) 50%, transparent 70%),var(--glass-bg)`;
    });
  ...
  document.querySelectorAll('.char-animate').forEach(el => {
    if (el.classList.contains('line2')) {
      el.style.cssText=`animation:line2Enter 0.8s cubic-bezier(0.34,1.56,0.64,1) both;animation-delay:0.35s;`;
      return;
    }
    ...
    [...text].forEach((char,i)=>{
      ...
      span.style.cssText=`display:inline-block;animation:charIn 0.5s cubic-bezier(0.34,1.56,0.64,1) both;animation-delay:${0.2+i*0.04}s;`;
      el.appendChild(span);
    });
  });
  ...
  charStyle.textContent=`
    @keyframes charIn{from{opacity:0;transform:translateY(30px) rotateX(-40deg)}to{opacity:1;transform:translateY(0) rotateX(0)}}
    @keyframes line2Enter{from{opacity:0;transform:translateY(36px) skewX(-4deg);filter:blur(6px)}to{opacity:1;transform:translateY(0) skewX(0);filter:blur(0)}}
  `;
  ...
  const counterObserver = new IntersectionObserver((entries) => {
    entries.forEach(e => {
      if (e.isIntersecting) {
        const el=e.target, val=el.dataset.count;
        if (!val||isNaN(val)) return;
        const target=parseInt(val); let current=0;
    ...
    entries.forEach(e=>{
      if(e.isIntersecting){
        flowBoxes.forEach((fb,i)=>{
          setTimeout(()=>{ fb.style.opacity='1'; fb.style.transform='none'; },i*120);
        });
  ...
  flowBoxes.forEach(fb=>{
    fb.style.opacity='0'; fb.style.transform='translateX(-20px)';
